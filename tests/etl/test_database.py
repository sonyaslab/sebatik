...
def sumber_database() -> dict:
    kepala_master = [
        "ID Indikator", "Kategori", "Kelompok / Pilar", "Arah Pembangunan", "Kode Indikator",
        ...
        "Frekuensi (RPJPD Provinsi)", "Status Metadata", "Perangkat Daerah Pengampu (Kaltara)",
        "Ketersediaan Data", "Periode Data", "Tahun Data Terakhir",
    ]
    master = [kepala_master]
    for nomor in range(1, 87):
        kategori = "ISV" if nomor <= 6 else "IUP"
        master.append(
            [f"{kategori}-{nomor:03d}", kategori, "Kelompok", "Arah", str(nomor), f"Indikator {nomor}",
             "Tidak", "Definisi", "Rumus", "Interpretasi", "BPS", "Tahunan", "Lengkap", "BPS",
             "Tersedia", "Tahunan", 2025]
        )
    nilai = [["ID Indikator", "Kategori", "Kelompok / Pilar", "Kode Indikator", "Nama Indikator (Kaltara)",
              "Jenis Nilai", "Tahun", "Nilai (Angka)", "Nilai (Teks Asli)", "Satuan/Catatan"]]
...
def test_loader_mengisi_database_kosong(session):
    ...
    session.flush()
    assert hasil == {"indikator": 86, "metadata_indikator": 86, "nilai_indikator": 1}
